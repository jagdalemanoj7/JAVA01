...
public class Tc_03 extends BaseClass{
	...
	@Test
	public void Tc_03Test() throws InterruptedException {
	...
	//  Enter the mandatory details
		WebElement rel = dr.findElement(By.id("related_to_type"));
		Select srel=new Select(rel);
		srel.selectByValue("Contacts");
		...
		while(itr.hasNext()) {
			...
			if(!parent.equals(child)) {
				...
				dr.findElement(By.xpath("//a[text()='Ganesh s']")).click();
			}
		}
	...
	//  Enter the mandatory details &select below OpportunityType [Existing Business,New Business] in Type dropdown & create a new Opportunity for every Type	
		WebElement type = dr.findElement(By.name("opportunity_type"));
		Select stype=new Select(type);
		stype.selectByValue("Existing Business");
			
	//  select below LeadSource[None,ColdCall,ExistingCustomer,Self Generated,Employee,partner,public Relations,DirectMail,conference,TradeShow,website,Word of mouth] in Lead Source dropdown & create a new Opportunity for every Lead source
		WebElement leadsrc = dr.findElement(By.name("leadsource"));
		Select slead=new Select(leadsrc);
		Thread.sleep(2000);
		slead.selectByVisibleText("Employee");
